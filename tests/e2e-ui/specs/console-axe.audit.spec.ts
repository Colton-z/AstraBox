...
/**
 * Open a route, allowing eight seconds for network idle before checking loading.
 *
 * Background refreshes can delay network idle. If that wait expires, allow
 * twenty seconds for loading indicators to disappear, with the route in the
 * failure message. scan separately requires a heading and no error surface.
 */
async function openSection(page: Page, route: string): Promise<void> {
  ...
  const idle = await page
    ...
    .catch(() => false);
  if (!idle) {
    await expect(page.locator('[data-slot="loading-state"]'), `${route} never finished loading`)
      .toHaveCount(0, { timeout: 20_000 });
  }
}

const HALVES = [
  { name: 'first half', pick: (all: string[]) => all.slice(0, Math.ceil(all.length / 2)) },
  { name: 'second half', pick: (all: string[]) => all.slice(Math.ceil(all.length / 2)) },
] as const;

for (const half of HALVES) {
test(`every console route passes WCAG 2.1 AA as axe measures it (${half.name})`, async ({ page }) => {
  test.setTimeout(170_000);
  const routes = half.pick(await routesFromSidebar(page));
