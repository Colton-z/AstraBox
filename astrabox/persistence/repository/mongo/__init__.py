...
#: Methods whose first argument (or ``filter=``) is a query filter.
...
def _explicit_nulls(query: Any) -> Any:
    """Make ``{field: None}`` match a stored null only, as the seam requires.

    MongoDB's null equality also matches a document without the field; the
    collection seam deliberately does not (see
    ``astrabox.testing.collection_conformance`` and the sqlite backend's
    ``query._eq``), because repositories write ``{"owner_id": None}`` to mean
    "explicitly null". ``$type: "null"`` is MongoDB's spelling of that.
    """
    ...
    for key, value in query.items():
        if key in _LOGICAL_OPERATORS and isinstance(value, list):
            translated[key] = [_explicit_nulls(item) for item in value]
        elif value is None and not key.startswith("$"):
            translated[key] = {"$type": "null"}
        else:
            translated[key] = value
    return translated


def _with_explicit_nulls(
    args: tuple[Any, ...], kwargs: dict[str, Any]
) -> tuple[tuple[Any, ...], dict[str, Any]]:
    if "filter" in kwargs:
        return args, {**kwargs, "filter": _explicit_nulls(kwargs["filter"])}
    if args:
        return (_explicit_nulls(args[0]), *args[1:]), kwargs
    return args, kwargs
...
class _RetryingCollectionProxy:
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        if not callable(attr):
            return attr
        filters = name in _FILTER_METHODS
