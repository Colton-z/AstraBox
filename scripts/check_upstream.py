...
# On 2026-09-22 the registry moved `cn` out of `lib/utils` into shadcn's new
# `cn` package (0.4.0, published that day), so every item below now imports it
# from "cn". The console keeps clsx + tailwind-merge behind `@/lib/utils` until
# that package has a release history: swapping the class merger of every
# component for a same-day 0.x release is a product risk, not a re-vendor.
# Each entry goes stale, and this check says so, once upstream moves off it.
_CN_PACKAGE = (
    'import { cn } from "cn"',
    'import { cn } from "@/lib/utils"',
    "the registry imports cn from the same-day `cn` 0.4.0 package; the "
    "console keeps clsx + tailwind-merge through @/lib/utils until it matures",
)
for _label in (
    "ui/alert-dialog.tsx",
    "ui/alert.tsx",
    ...
    ALLOWED_DRIFT.setdefault(_label, []).append(_CN_PACKAGE)

# Where "cn" sorts ahead of a sibling import, the same change also moves the
# import, so these three name the neighbouring import on both sides.
_CN_WHY = _CN_PACKAGE[2]
ALLOWED_DRIFT["ui/resizable.tsx"] = [
    (
...
ALLOWED_DRIFT.setdefault("ui/calendar.tsx", []).extend([
    ('import * as React from "react"\nimport { cn } from "cn"', 'import * as React from "react"', _CN_WHY),
    (
...
ALLOWED_DRIFT.setdefault("ui/tabs.tsx", []).append(
    (
        'data-slot="tabs-content"\n      className={cn("flex-1 text-sm outline-none", className)}',
        'data-slot="tabs-content"\n'
        "      // The panel takes focus so a keyboard reader can scroll it, and a stop\n"
        "      // that looks identical focused and unfocused cannot be found by the\n"
        "      // reader it exists for (the non-text contrast floor is 3:1).\n"
        "      // Same ring as the trigger above rather than a second one.\n"
        "      className={cn(\n"
        '        "flex-1 text-sm outline-none focus-visible:ring-[3px] '
        'focus-visible:ring-ring/50 focus-visible:outline-1 focus-visible:outline-ring",\n'
        "        className,\n"
        "      )}",
        "the registry's tab panel is focusable but draws no focus indicator; "
        "the console gives it the trigger's ring (3:1 floor, 7e0d78d3)",
    )
)
